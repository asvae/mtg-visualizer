> **Deprecated (2026-09-04).** Superseded by `functional-model/` — a declarative `CardDefinition`/`Effect` representation run through real, mutable game state (`functional-model/state.ts`), where a synergy fact is *derived by actually executing the card* (see `functional-model/scripts/find-synergies.mjs`) rather than hand-authored or translated from a Forge script one abstraction layer removed from real behavior. The judgment call driving this: this schema's own node/flow graph is considered inadequate next to that — harder to verify, and never actually run. Still kept live on the card page (the "Synergy model" column) and still load-bearing for `server/api/card/[set]/[number].ts`'s synergy-fallback and the Interactions panel (both read `forge-model/` — see that folder's own README, also deprecated) — not scheduled for removal yet, just not the direction new work goes. New cards should be modeled in `functional-model/cards/`, not added here.
