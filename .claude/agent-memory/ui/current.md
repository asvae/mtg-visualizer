...
- [Engine-support filter + badge](topics/engine-support-filter-badge.md) — pattern for layering a second single-select filter axis on top of `useStatusFilterList` without touching it; `CardStatusPageEntry` gained `engineSupport` passthrough.
