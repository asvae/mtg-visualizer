...
- [Engine-support filter + badge](topics/engine-support-filter-badge.md) — pattern for layering a second single-select filter axis on top of `useStatusFilterList` without touching it; `CardStatusPageEntry` gained `engineSupport` passthrough; nav-row second swatch (enlarged status dot + on/off/neutral engine-support dot) added same file.
- [Engine console card-match chip](topics/engine-console-card-match-chip.md) — real thumbnail+name+link chip for name-only match lists (Sinks' Real FDN pool matches); dynamic `:is="'NuxtLink'"` string form silently fails to hydrate, use real v-if/v-else branches instead.
