...
**Flagged, not forced: Ajani's Pridemate does NOT categorize under `"Lifegain"` under this design, contrary to what may have been assumed when this catalog-first work was scoped.** This isn't an oversight — it's the same, real structural finding `card-interactions.ts`'s own header comment already documented BEFORE the catalog existed (see this contract's earlier `computeCardInteractions` section, "Real design investigation"): `Trigger.name` (`'onLifeGained'`) is a free-text label with no safe, closed-vocabulary way to derive "this trigger's own firing precondition is a lifegain event" — `Trigger.on`'s closed enum has no lifegain member, and FDN cards carry no `oracleText` to fall back on the way `recognizers/lifegain-trigger-structural.ts` does for the equivalent FIN cards. Making a catalog entry's own `query` real did not change this: `entry.query` is a PRODUCER-shaped query (matched against a candidate's own `deriveOccurrences`), and Ajani's Pridemate is structurally a CONSUMER of lifegain, not a producer of it — there is no existing primitive in this codebase that checks "does this card's own trigger react to category X," and building one safely would require either (a) trusting the free-text trigger name (explicitly declined, twice now, same reasoning both times), or (b) a genuine `Trigger.on` closed-vocabulary addition with real engine wiring (a materially bigger change than "derive on the fly," out of scope here). Getting Ajani specifically into a `"Lifegain"` category needs one of those two, not attempted in this pass — flagged for the orchestrator/user rather than guessed at with a wrong category, per this project's own "Facts must be recognizer-derived" convention.

Tests: `functional-model/card-interactions.test.ts` gained cases for the catalog-first path (Day of Judgment's real `"Graveyard fodder"` catalog categorization, a mocked producer card matching `lifegain` by catalog label instead of the raw `"life gain"` label, and a real, honest assertion that Ajani's Pridemate's own categories still do NOT include `"Lifegain"` — see the writeup above).
