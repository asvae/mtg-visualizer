...
**Known, narrow, out-of-scope gap flagged, not fixed at the time — since FIXED, 2026-09-18, later still, by `schema`**: `server/api/sink-catalog/index.get.ts`'s own `computeRealMatches` (`hasConsumerSignal`) only checked `entry.consumerTriggerNames`/`entry.consumerTriggerOn` — it didn't yet know about `consumerBattlefieldPresence`, so the `/app/engine/sinks` review-tool page's own `consumerMatches` list was `undefined` (omitted, not a crash) for both new entries even though they DO have a real consumer signal; `producerMatches` (8/72) still rendered correctly. Fixed as part of the same pass that repaired the `3a73aabf` family/instance slug regression below (`computeRealMatches` now checks a callable `SinkInstance`'s own uniform match-detail return, which already answers `consumerBattlefieldPresence` alongside every other consumer signal, for free) — see "Family/instance slug regression fix" further down for the live-verified numbers.
...
**Verified live** (real dev server, `GET /api/card/fdn/15`): `functionalModel.cardInteractions` now includes `{category: 'Same-name copies', count: 1, matches: [{card: 'Hare Apparent', self: true, ...}]}`. `GET /api/sink-catalog`: new entry `battlefield-presence-hare-apparent` shows distinctly, `blue` (corpus manifest `7/7`), `realMatches.producerMatches: ['Hare Apparent']` only (confirming the bespoke-literal-name producer design — no OTHER real FDN card is literally named "Hare Apparent"); `consumerMatches` is omitted, same pre-existing, already-flagged `computeRealMatches`/`consumerBattlefieldPresence` gap section 9 already documents (not fixed here either, still `card`/`server`-owned, out of scope for this `engine`-scoped task) — **historical**: both this gap and `battlefield-presence-hare-apparent`'s own standing as an independently-served row are superseded by the `3a73aabf` family/instance refactor + its follow-up regression fix (see "Family/instance slug regression fix" further down) — `GET /api/sink-catalog` now serves one combined `battlefield-presence` family row (all 3 members) with real, non-empty `consumerMatches`, not this slug on its own.
...
### 10. `server/api/sink-catalog/index.get.ts` — family/instance slug regression fix (2026-09-18, later still)

The `3a73aabf` family/instance factory refactor (see `sink-catalog-status.ts`'s own header, "Family-scoped review status") deliberately left this route untouched, flagging (both in that file's own header and this contract's section 9 above) that it would break once `computeSinkCatalogStatus()` started returning ONE row per real FAMILY (`slug: 'battlefield-presence'`/`'counters'`) instead of one row per real `SINK_CATALOG` instance slug. It broke worse than the flag anticipated: `SINK_CATALOG.find((e) => e.slug === entry.slug)` came back `undefined` for BOTH family rows (no real `SINK_CATALOG` member's own `.slug` equals a family key), so `realMatches` was silently `undefined` ("Not computed") for `battlefield-presence`/`counters` entirely — not just the `consumerBattlefieldPresence` cosmetic gap section 9 flagged, the WHOLE "Real FDN pool matches" section.

**Fix**: `entry.evidence.members[].slug` (`sink-catalog-status.ts`'s own real per-instance evidence array, length 1 for a singleton, N for a family) is the real, always-correct list of `SINK_CATALOG` member slugs a served row aggregates — looked up per member instead of looking up `entry.slug` itself. `computeRealMatches` now takes an ARRAY of real member instances and unions/dedupes (by card name) each member's own producer/consumer match across the whole array — a card matching via more than one member (e.g. a Cat-token-maker satisfying both `battlefield-presence-cats` and `-creatures`) counts once. Producer/consumer matching per member goes through a real, uniform callable `SinkInstance` check (`instance(candidate, root)`) when the member is callable (every `battlefield-presence-*`/`counters-*` instance, built by the `BattlefieldPresenceSink`/`CountersSink` factories) — which ALSO fixes section 9's own separately-flagged `consumerBattlefieldPresence` gap for free, since a callable instance's own match-detail already answers every consumer signal uniformly, not just `consumerTriggerNames`/`consumerTriggerOn`. A plain, non-callable singleton (`lifegain`/`graveyard-fodder`/`etb`) falls back to the exact pre-fix `matchSink`/`matchesConsumerTriggerNames`/`matchesConsumerTriggerOn` checks, unchanged. `loadSourceFiles`'s own `corpusManifest` (`evidence.corpusManifestPath`'s own doc-commented "first member only, not fully general" limitation) is fixed the same way: a real multi-instance family now combines EVERY member's own real `<slug>.corpus.json` into one valid JSON object (`{total, passing, members: {<slug>: <parsed content>}}`), not just the first member's; a singleton is served exactly as before (bare single-file content, unchanged shape). `query` (no single honest "the" query exists for a whole family) falls back to the first real member's own `query`, same "representative member" convention `evidence.corpusManifestPath` already established pre-fix.

**Verified live**, real dev server, `GET /api/sink-catalog`: `battlefield-presence` — `blue`, `realMatches.producerMatches.length: 111`, `realMatches.consumerMatches.length: 4` (`Claws Out`, `Dauntless Veteran`, `Hare Apparent`, `Preposterous Proportions`); `counters` — `blue`, `realMatches.producerMatches.length: 19`, `realMatches.consumerMatches.length: 1` (`Exemplar of Light`); `sourceFiles.corpusManifest` for `battlefield-presence` is now a real combined `{total:31, passing:31, members:{battlefield-presence-cats:..., battlefield-presence-creatures:..., battlefield-presence-hare-apparent:...}}` object (was, pre-fix, the Cats instance's own `11/11` slice only); `counters` (still a 1-member group today) serves its pre-existing bare single-file shape unchanged (`{total:12, passing:12, cases:[...]}` — no `members` wrapper), confirming the singleton-degeneration path is a true no-op. `npx vitest run functional-model`: 120 files, 1338 passed / 5 skipped (identical to the `3a73aabf` baseline — zero regression, `sink-catalog-status.ts`/`sink-model/**` untouched by this pass, only the one server route). `npm run typecheck`: identical known 7-diagnostic baseline (`CardDetailTabs.vue`×3/`card-status.ts`/`card.ts`/`mana.ts`/`server/api/tokens/by-key.ts`), zero new errors — 2 incidental `as unknown as SinkInstance` casts were needed in the new route code itself (`SINK_CATALOG`'s own inferred element type collapses every member, callable or not, down to the plain non-callable `SinkCatalogEntry` shape — `typeof instance === 'function'` is still runtime-correct, TS just needs the cast to recover the call signature statically).

`app/pages/app/engine/sinks/[[slug]].vue`/`EngineConsoleCardMatchChip.vue` (both `ui`-owned, untouched by this pass) needed zero changes — confirmed: both already render whatever `realMatches.producerMatches`/`consumerMatches`/`sourceFiles.corpusManifest` they're served, and now receive the real, non-empty values for both family rows instead of `undefined`/a partial slice.
