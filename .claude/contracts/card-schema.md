...
**Fixed (2026-09-18, follow-up pass)**: `POST /api/fdn-cards/:slug/review` now also checks `isSinkAttachmentComplete` (re-derived against the fresh gate result, not the stored file, so "reject-after-confirm"/"confirm-after-reject" keeps working) — 400s with a clear message if attachment isn't done yet. Live-verified: `helpful-hunter` (gate passes, no attachment) 400s on review; after `POST /api/fdn-cards/helpful-hunter/sinks {"attachedSlugs":[]}`, the identical review request then succeeds. A card can no longer be reviewed `yellow`/`green` without its attachment step having happened.

### 5. Sink catalog + per-card Sinks UI — built (2026-09-18, follow-up pass)

- **Catalog review UI**: new engine-console tab (`Cards | Predicates | Features | Sinks`), `app/pages/app/engine/sinks/[[slug]].vue`, mirroring the Predicates page's structure exactly (list panel, code sections, Confirm/Reject…/Clear review). Backed by `GET /api/sink-catalog` and `POST /api/sink-catalog/review` (mirrors `server/api/sink-derivations`'s own routes byte-for-byte in shape), writing to `functional-model/sink-catalog-reviews.json` (tracked, sibling to `sink-derivation-reviews.json`).
- **Per-card attachment UI**: new "Sinks" tab on the FDN card page (`CardDetailTabs.vue`'s `isFdn` strip: `Scenarios? | Sinks | Card Definition`) — attachment-status badge, a checkbox picker over `SINK_CATALOG`, and its own "Mark attachment reviewed" action (separate from the card-level pipeline Confirm/Reject). Backed by new `POST /api/fdn-cards/:slug/sinks`, body `{ attachedSlugs: string[] }`, 400s on any slug not in `SINK_CATALOG`. `server/api/card/[set]/[number].ts`'s `FunctionalModelData` gained `sinkAttachment`/`sinkAttachmentStatus` (both `fdn`-only, mirrored into `app/lib/cardResponse.ts`).
- Only 2 real catalog entries (`lifegain`, `graveyard-fodder`) — growing the catalog is separate future authoring work.
