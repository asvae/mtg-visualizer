...
`data/themes.json` (same precedent as Planeswalker/Battle/Copy/Tokens — a
...
added to `themes.json` by hand instead of purely auto-generating.
...
case, don't forget to set `"reviewed": true`** on the entry once you're
...
applicable card has SOME entry.
...
somehow does, still write an entry for it (`{ "themes": {}, "reviewed": true }`
— empty themes, but present and marked reviewed) rather than leaving it out.
`reviewed: true` is what marks a card as actually reviewed now (see
`REVIEW_PROCESS.md`'s card-selection step) — presence alone no longer means
that, since `scripts/prefill-main-types.mjs` gives most cards a mechanical-only
entry with no `reviewed` field at all. Cards with no entry at all show as
"Not Processed" in the UI — that's a pending-review marker `buildGraph.ts`
adds automatically, not something you write.
...
7. Write the result to `data/fin/fin_relations.json`, `"reviewed": true` included
