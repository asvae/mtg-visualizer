...
/** Builds a `FactRow` for a fact identified by `id` alone, optionally
 * anchored to a real (line, start) position via `Fact.annotations` — the
 * Nth distinct `line` given across a test's own fixtures models the Nth
 * printed line a real card's oracle text would put a fact at (`end` is
 * irrelevant to ordering, filled with a dummy value one past `start`). No
 * `annotations` at all models a fact with no real textual anchor (no
 * `sourceText`/`highlight` authored, or the match never landed). */
function row(id: string, opts?: { line?: number; start?: number; role?: 'source' | 'sink' }): FactRow {
  const { line, start = 0, role = 'source' } = opts ?? {};
  const fact = {
    id,
    role,
    event: 'placeholder',
    ...(line !== undefined ? { annotations: [{ target: 'oracle', line, start, end: start + 1 }] } : {}),
  } as unknown as Fact;
  return { fact, key: id };
}
...
describe('orderByTextPosition', () => {
  it('reorders fin/279 The Gold Saucer\'s own facts to printed oracle-text order', () => {
    ...
    // last). Real printed line order: {T}: Add {C}. / {2},{T}: Flip a
    // coin... / {3},{T}, Sacrifice two artifacts: Draw a card. — only
    // these 3 facts ever get a real textual anchor on this card.
    const rows = [
      row('played'),
      row('battlefield-presence'),
      row('mana', { line: 0 }),
      row('coin-flip', { line: 1 }),
      row('treasure-battlefield-presence'),
      row('treasure-enters-battlefield'),
      row('draws-a-card', { line: 2 }),
      row('artifact-sink', { role: 'sink' }),
    ];

    const result = orderByTextPosition(rows).map((r) => r.key);
  ...
  it('leaves wholly-unanchored facts in plain authored order (no sink-first jump) when NOTHING in the group has a real anchor', () => {
    ...
    const rows = [row('token-creation'), row('put-counter'), row('wants-lands', { role: 'sink' })];

    expect(orderByTextPosition(rows).map((r) => r.key)).toEqual(['token-creation', 'put-counter', 'wants-lands']);
  });

  it('passes a simple single-fact card through unchanged (e.g. fin/271 Adventurer\'s Inn)', () => {
    const rows = [row('lifegain')];
    expect(orderByTextPosition(rows).map((r) => r.key)).toEqual(['lifegain']);
  });

  it('sorts a sink before a source when both are anchored to the exact same real line', () => {
    const rows = [row('effect', { line: 0, start: 0, role: 'source' }), row('cost', { line: 0, start: 0, role: 'sink' })];
    expect(orderByTextPosition(rows).map((r) => r.key)).toEqual(['cost', 'effect']);
  });

  it('inherits a preceding anchored position across MULTIPLE consecutive unanchored facts', () => {
    const rows = [row('anchored-1', { line: 0 }), row('unanchored-a'), row('unanchored-b'), row('anchored-2', { line: 1 })];
    ...
    expect(orderByTextPosition(rows).map((r) => r.key)).toEqual(['anchored-1', 'unanchored-a', 'unanchored-b', 'anchored-2']);
  });
  ...
  it('reorders a fact whose real anchor appears LATER in oracle text ahead of one authored earlier but anchored sooner', () => {
    ...
    const rows = [row('second-ability', { line: 1 }), row('first-ability', { line: 0 })];
    expect(orderByTextPosition(rows).map((r) => r.key)).toEqual(['first-ability', 'second-ability']);
  });
});
