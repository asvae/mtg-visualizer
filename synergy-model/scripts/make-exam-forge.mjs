// Same round-trip fidelity packet as make-exam.mjs (EXAM_PROCESS.md), for a
// card whose synergy nodes come from app/lib/forgeTranslate.ts (a
// Forge-script translation, forge-model/data/<slug>.txt) instead of a
// hand-authored synergy-model/data/edges.json entry — there is no bundled
// Scryfall file for these cards' sets, so the self registry is built from a
// live Scryfall lookup by name instead of data/fin/fin_scryfall.json.
//
// Usage: node synergy-model/scripts/make-exam-forge.mjs "<card name>"
//   Writes synergy-model/exams/<slug>.md — same packet shape, same loop
//   (EXAM_PROCESS.md), same synergy-model/exams/<slug>.result.json output.

import { readFile, writeFile, mkdir } from 'node:fs/promises';

const query = process.argv.slice(2).join(' ').trim();
if (!query) {
  console.error('Usage: node synergy-model/scripts/make-exam-forge.mjs "<card name>"');
  process.exit(1);
}
...
if (!forgeRaw) {
  console.error(`No forge-model/data/${slug}.txt — this card hasn't been prepared (see forge-model/README.md).`);
  process.exit(1);
}

const { parseForgeScript } = await import('../../app/lib/forgeScript.ts');
const { translateForgeCard } = await import('../../app/lib/forgeTranslate.ts');
const translated = translateForgeCard(parseForgeScript(forgeRaw));
const entry = { name: translated.name, nodes: translated.nodes, flow: translated.flow };

const registries = await readFile('synergy-model/data/registries.json', 'utf8');

const scryRes = await fetch(`https://api.scryfall.com/cards/named?exact=${encodeURIComponent(query)}`, {
  headers: { 'User-Agent': 'mtg-visualizer/0.1', Accept: 'application/json' },
});
if (!scryRes.ok) {
  console.error(`Scryfall lookup for "${query}" failed (${scryRes.status}) — needed for the self registry.`);
  process.exit(1);
}
const scryfallEntry = await scryRes.json();
...
// Same schema summary and packet template as make-exam.mjs, verbatim — the
// two scripts must stay in lockstep here (see EXAM_PROCESS.md); if this
// drifts, re-sync by diffing against make-exam.mjs's own copy.
...
if (translated.unmapped.length) {
  console.log(`Note: translator left ${translated.unmapped.length} unmapped entr${translated.unmapped.length === 1 ? 'y' : 'ies'} for this card: ${JSON.stringify(translated.unmapped)} — the packet only reflects what DID translate.`);
}
