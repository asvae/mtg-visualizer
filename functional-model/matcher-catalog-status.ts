// Matcher CATALOG entry status — a review axis for `functional-model/
// matcher-model/catalog/*.ts` entries (the shared, curated `MatcherQuery`
...
// This file instead tracks whether a CATALOG ENTRY's own curated `MatcherQuery`
...
//
// **Discovery is the real, statically-imported `MATCHER_CATALOG` array**
// (`matcher-model/catalog/index.ts`), NOT a second hand-seeded metadata array
...
// not-yet-authored catalog entry simply isn't in `MATCHER_CATALOG` at all yet
...
// cosmetic)** — see `MatcherCatalogEntry.family`'s own doc comment
// (`matcher-model/catalog/entry.ts`) for the full reasoning. Every real
// `MATCHER_CATALOG` entry (a "sink INSTANCE") is grouped by `entry.family ??
...
// exactly ONE `MatcherCatalogStatusEntry` for the whole group — its own
...
// corpus coverage (see `computeMatcherCatalogStatus` below) — a human review
// verdict (`matcher-catalog-reviews.json`, keyed the same way, by group key)
...
// MATCHING (`Matcher`'s own `entry(candidate)` call — `card-
// interactions.ts`'s per-`MATCHER_CATALOG`-entry loop, the server API route's
...
// `MATCHER_CATALOG.find((e) => e.slug === entry.slug)` came back `undefined`
// for BOTH real family rows (`'battlefield-presence'`/`'counters'` match NO
// single `MATCHER_CATALOG` entry's own `.slug`), silently killing that route's
...
// half of this same gap — `MatcherCatalogEvidence.corpusManifestPath`'s own
...
// `computeMatcherCatalogStatus()`'s own returned `.slug` field directly (never
// a separate `MATCHER_CATALOG.find` lookup the way `index.get.ts`'s bug
...
import type { MatcherCatalogEntry } from './matcher-model/catalog/entry';
import { readFunctionalModelFile } from './source-files';

export type MatcherCatalogBaseline = 'gray' | 'purple' | 'blue';
export type MatcherCatalogColor = 'gray' | 'purple' | 'blue' | 'yellow' | 'green' | 're-review';

export interface MatcherCatalogCorpusManifest {
  total: number;
  passing: number;
}

/** Real, per-INSTANCE corpus evidence — one per real `MATCHER_CATALOG` member
 ...
 * top-level `MatcherCatalogEvidence` fields — a consumer that needs the real
 * per-instance breakdown (e.g. "which ONE of Battlefield presence's 3
 * instances isn't fully passing yet") reads this array. */
export interface MatcherCatalogMemberEvidence {
  slug: string;
...
export interface MatcherCatalogEvidence {
   ...
   * singleton group. */
  members: MatcherCatalogMemberEvidence[];
}

export interface MatcherCatalogStatusEntry {
   ...
   * for a singleton — mirrors `MatcherCatalogEntry.consumerTriggerNames`'s own
 ...
 * `MATCHER_CATALOG` themselves already follow. */
...
interface CatalogGroup {
   ...
   * been authored yet. See `MatcherCatalogEntry.family`'s own doc comment
   * (`matcher-model/catalog/entry.ts`). */
  isFamily: boolean;
  members: MatcherCatalogEntry[];
}

/** Groups every real `MATCHER_CATALOG` entry by `entry.family ?? entry.slug` —
...
function groupCatalogByFamily(): CatalogGroup[] {
  const order: string[] = [];
  const groups = new Map<string, MatcherCatalogEntry[]>();
 ...
 * since there's only ever one instance). See `computeMatcherCatalogFingerprint`
...
function loadCorpusManifest(path: string): MatcherCatalogCorpusManifest {
...
function memberEvidenceFor(entry: MatcherCatalogEntry, root: string): MatcherCatalogMemberEvidence {
 ...
 */
export function computeMatcherCatalogStatus(root: string = process.cwd()): MatcherCatalogStatusEntry[] {
  return groupCatalogByFamily().map((group): MatcherCatalogStatusEntry => {
    ...
    let baseline: MatcherCatalogBaseline;
    ...
    return {
      ...
      // `counters-*` today — may omit it, per `MatcherCatalogEntry.query`'s own
...
// server route).
const REVIEWS_RELATIVE_PATH = join('functional-model', 'matcher-catalog-reviews.json');

interface MatcherCatalogReviewVerdictOnly {
  verdict?: 'confirm' | 'reject';
  fingerprint?: string;
}

function loadReviewVerdicts(root: string): Record<string, MatcherCatalogReviewVerdictOnly> {
 ...
 * pass, one real `MATCHER_CATALOG` entry's own `.slug`) OR a real GROUP/family
 * key (what a future review-status consumer keyed on
 * `computeMatcherCatalogStatus()`'s own `slug` field would pass) — to the
 * group key `computeMatcherCatalogStatus`/review storage actually use. An
 ...
 * `isMatcherCatalogEntryUsable(entry.slug, root)` call (looping per real
 * `MATCHER_CATALOG` instance) keep working verbatim while review status itself
 * is now computed per family underneath it. Falls back to treating an
 * unrecognized key as already-a-group-key (matches `computeMatcherCatalogStatus`'s
 * own "unknown slug -> gray, never throws" convention below). */
function resolveGroupKey(key: string): string {
  const found = MATCHER_CATALOG.find((e) => e.slug === key);
 ...
 * inputs `computeMatcherCatalogStatus` itself reads to decide gray/purple/blue
 ...
 */
export function computeMatcherCatalogFingerprint(key: string, root: string = process.cwd()): string | null {
 ...
 * `resolveGroupKey`. Uncached — see `isMatcherCatalogEntryUsable` below for the
...
export function computeMatcherCatalogColor(key: string, root: string = process.cwd()): MatcherCatalogColor {
  const groupKey = resolveGroupKey(key);
  const entry = computeMatcherCatalogStatus(root).find((e) => e.slug === groupKey);
  const baseline: MatcherCatalogBaseline = entry?.baseline ?? 'gray';
  ...
  if (review?.verdict === 'confirm') {
    const currentFingerprint = computeMatcherCatalogFingerprint(groupKey, root);
...
// tracked entry on every consultation of `isMatcherCatalogEntryUsable`, static
...
// `computeMatcherCatalogStatus()` pass, already itself cheap and already
// re-run per distinct cache miss today).
const colorCache = new Map<string, Map<string, MatcherCatalogColor>>();

function cachedColor(key: string, root: string): MatcherCatalogColor {
  ...
  if (color === undefined) {
    color = computeMatcherCatalogColor(key, root);
    cache.set(key, color);
  }
  return color;
}
 ...
 * production code. */
export function resetMatcherCatalogColorCacheForTests(): void {
  colorCache.clear();
}
 ...
 */
export function isMatcherCatalogEntryUsable(key: string, root: string = process.cwd()): boolean {
