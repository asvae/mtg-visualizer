// Sink catalog entry: ETB ("blink/bounce value" — see below for what this
...
//   `event:'bounce'` occurrence in `match-sink.ts`'s `walkEffects`'s own
...
//   (`match-sink.ts`) — a sibling to `lifegain.ts`'s own `consumerTriggerNames`
...
// documented future gap — widen `match-sink.ts`'s own `case 'move'` the next
