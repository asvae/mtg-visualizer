// Sink-only synergy matching — prototype (2026-09-17). See `match-query.ts`'s
// own header for the full design writeup; this file is just the query
// shape itself.
//
// A `MatcherQuery` is today's `synergy.ts` `Fact` shape a card author already
...
export type MatcherQuery = Omit<Fact, 'annotations' | 'provenance' | 'role' | 'triggeredBy'> & {
