...
// Fact at all. A `MatcherQuery` (`matcher-query.ts`) stays a small, curated,
...
// occurrence family — `sink-derivation-predicates/*.ts`'s own hand-written
...
// matcher (`occurrenceSatisfiesQuery` below) does NOT reproduce that bug — it
...
import { crewTapOccurrences } from '../sink-derivation-predicates/crew';
...
import { lifelinkProductionOccurrences } from '../sink-derivation-predicates/lifelink';
...
// `deriveOccurrences`/`matchQuery`, so a not-yet-blue predicate can still be
...
// `match-query.test.ts`'s own gate test(s) for a live before/after proof.
...
export interface ProducerOccurrence extends Constraints {
  ...
  /**
   * True iff this occurrence came from a `sink-derivation-predicates/*.ts`
...
function collectForFace(face: CardDefinition, faceLabel: 'front' | 'back', out: ProducerOccurrence[]): void {
  ...
  // off `CardDefinition.triggers[].on` (`MatcherCatalogEntry.consumerTriggerOn`,
 ...
 * (see `match-query.test.ts`'s own gate test(s)). */
...
function isGraveyardArrivalWant(w: MatcherQuery): boolean {
 ...
 * `graveyard-fodder` corpus, `matcher-model/catalog/graveyard-fodder.test.ts`):
 ...
 * fix, since the zone-shaped branch in `occurrenceSatisfiesQuery` only ever
 ...
 * of this destroy" — a `MatcherQuery` has no such reference (deliberately, per
 * `matcher-query.ts`'s own header: a curated query, not tied to one card).
 ...
 */
function satisfiesDestroyImpliesDies(p: ProducerOccurrence, w: MatcherQuery): boolean {
...
function occurrenceSatisfiesQuery(p: ProducerOccurrence, w: MatcherQuery, candidate: CardDefinition): boolean {
  ...
  if (w.target === 'self') {
    ...
    // sink's own owning card, which a `MatcherQuery` deliberately doesn't carry
 ...
 * — see sink F in `match-query.test.ts`: a program-AST-derived broadcast
...
export interface MatchResult {
   ...
   * `sink-derivation-predicates/*.ts` sink-derivation predicate (Saga/Crew/
 ...
 * authored `source` Fact involved) satisfy `query`? `root` — see
 * `deriveOccurrences`'s own doc comment — defaults to `process.cwd()`. */
export function matchQuery(query: MatcherQuery, candidate: CardDefinition, root: string = process.cwd()): MatchResult {
  for (const occ of deriveOccurrences(candidate, root)) {
    if (occurrenceSatisfiesQuery(occ, query, candidate)) return { matched: true, via: occ.via, predicateDerived: occ.predicateDerived };
  }
  return { matched: false };
}

/** Count of OTHER cards in `pool` that satisfy `query` — the aggregate this
 * task's own "for a given sink, count of other CardDefinitions that satisfy
 * it" ask names. `pool` should already exclude `query`'s own owning card if
 ...
 * existing pipeline). */
export function countMatchesForQuery(query: MatcherQuery, pool: CardDefinition[]): number {
  return pool.filter((c) => matchQuery(query, c).matched).length;
}

/** The reverse direction: for a given candidate, how many of `queries` does
 * its OWN structurally-derived occurrences satisfy — the task's own
 * "reverse count" ask, using the exact same `matchQuery` primitive from the
 * other side. */
export function countQueriesSatisfiedByCard(candidate: CardDefinition, queries: MatcherQuery[]): number {
  return queries.filter((query) => matchQuery(query, candidate).matched).length;
}

/**
 * Real, structural CONSUMER-side signal (2026-09-18) for a catalog entry's
 * own `MatcherCatalogEntry.consumerTriggerNames` (`catalog/entry.ts`) — does
 ...
 * Deliberately a standalone function (not folded into `matchQuery`, which
 * stays scoped to `MatcherQuery`-vs-`ProducerOccurrence` producer matching) so
 * `match-query.ts` itself never has to import the `catalog/` module —
 * `card-interactions.ts` is the one real caller that already knows about
 * both `MATCHER_CATALOG` and this function.
 */
 ...
 * `MatcherCatalogEntry.consumerTriggerOn` (`catalog/entry.ts`) — does
 ...
 * mechanic — see `MatcherCatalogEntry.consumerBattlefieldPresence`'s own doc
 ...
 * doesn't go through `deriveOccurrences`/`matchQuery` at all — same
