...
// hare-apparent}` matcher catalog entries — 3 configurations of ONE shared
// factory (`BattlefieldPresenceMatcher`, `families/battlefield-presence.ts`) as
...
import { matchesBattlefieldPresenceConsumer, matchQuery } from '../match-query';
...
describe('battlefield-presence-cats matcher catalog entry (mocked CardDefinition fixtures)', () => {
  ...
  it('CONSUMER mode: matches a real board-COUNTED cost reduction, "Affinity for Cats" (the real Claws Out, FDN #6, shape)', () => {
    ...
    expect(matchQuery(query, card).matched).toBe(false); // an Instant is never itself a Cat producer
  });
  ...
  // -------------------------------------------------------------------------
  // Callable Matcher contract (2026-09-18, new with the factory refactor) — the
  ...
  // `SinkMatchDetail | null` — see `entry.ts`'s own `Matcher` doc
...
describe('battlefield-presence-creatures matcher catalog entry (mocked CardDefinition fixtures)', () => {
  ...
  it('CONSUMER mode: matches a BARE (no-subtype) pumpAll — the real Claws Out (FDN #6) "Creatures you control get +2/+2" shape', () => {
    ...
    expect(matchesBattlefieldPresenceConsumer(entry.consumerBattlefieldPresence, card)).toBe(true);
    expect(matchQuery(query, card).matched).toBe(false); // an Instant is never itself a creature producer
  });
...
describe('battlefield-presence-hare-apparent matcher catalog entry (mocked CardDefinition fixtures)', () => {
