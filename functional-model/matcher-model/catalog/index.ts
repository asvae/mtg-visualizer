// The sink catalog — the ONE shared, reviewed list of curated `MatcherQuery`
...
// `MATCHER_CATALOG` genuinely has a real, existing catalog module backing it
...
// matcher-catalog-status.ts` computes each entry's own gray/purple/blue/
...
//
// **2026-09-18, `BattlefieldPresenceMatcher`/`CountersMatcher` factory refactor**
...
// `BattlefieldPresenceMatcher` factory living in `catalog/families/battlefield-
// presence.ts` (`counters-plus1plus1` likewise from `CountersMatcher`,
...
// for the full factory writeup, and `entry.ts`'s `MatcherCatalogEntry.family`/
// `Matcher`/`MatcherFamily` doc comments for the real vocabulary
// (`MATCHER_CATALOG` itself is still a flat array of individual sink
// INSTANCES, review status is what's now grouped by family —
// `matcher-catalog-status.ts`).
...
export type { MatcherCatalogEntry } from './entry';

export const MATCHER_CATALOG = [
  lifegain,
  graveyardFodder,
  etb,
