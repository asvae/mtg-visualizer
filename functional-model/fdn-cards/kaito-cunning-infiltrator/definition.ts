...
export const kaitoCunningInfiltrator: CardDefinition = {
  ...
  // Kaito." No `on` value exists for a combat-damage-to-player event;
  // kept as a name-only trigger (same convention every other not-yet-
  // auto-fired trigger in this pool already uses). The loyalty counter
  // itself is a real, mechanically tracked `putCounter` (not a
  // placeholder — `state.ts`'s own persistent per-card counter map is
  // generic to any counterType string).
  triggers: [
    {
  ...
  // free-text constraint is mechanically enforced" convention gogo-
  // master-of-mimicry's own `{X}{X}, {T}` cost already establishes), named
  // abilities a scenario can invoke directly rather than a real
  ...
  abilities: [
    ...
    },
    {
      ...
      effects: [
        {
          // Real Forge: `Triggers$ CastTrig | ... SpellDescription$ You get
          // an emblem with "Whenever a player casts a spell, you create a
          // 2/1 blue Ninja creature token."` GENUINE CAPACITY GAP: no
          // emblem mechanic exists anywhere in this engine. Documented
          // no-op placeholder, not folded into anything else.
          kind: 'custom',
          describe:
