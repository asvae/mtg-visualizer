...
export function recognizePumpAllAttackingEffectStructural(input: StructuralRecognizerInput): RecognizerResult {
  const all = allEffects(input).filter((e): e is PumpAllEffect => e.kind === 'pumpAll' && e.predicate === 'attacking-creatures');
  if (all.length === 0) {
  ...
  for (const effect of qualifying) {
    ...
    const pattern = new RegExp(`\\bAttacking creatures (get ${numbers})${suffix}\\b`, 'i');
    ...
    facts.push({
      role: 'source',
      fact: { event: 'pump', target: { types: { has: ['Creature'] }, attacking: true }, annotations: [annotation] },
      provenance: { origin: 'parser', rule: RULE },
    });
    facts.push({
      role: 'sink',
      fact: { to: 'Battlefield', types: { has: ['Creature'] }, attacking: true, annotations: [annotation] },
      provenance: { origin: 'parser', rule: RULE },
    });
  }
