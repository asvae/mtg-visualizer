// Query-scoped card subset for the "scryfall filter" URL leg (see store.ts's
// `sf` param): resolves an arbitrary Scryfall search against the live Scryfall
// API, then intersects the matches against the already-tagged
// data/global_relations.json / data/global_themes.json corpus, and hands the
// frontend back just enough to run through buildGraph.ts — same shape as the
// per-set static files, just built on the fly instead of checked in.
//
// GET /api/cards?q=<scryfall search syntax>

import relationsData from '../../data/global_relations.json' with { type: 'json' };
import themesData from '../../data/global_themes.json' with { type: 'json' };
...
export default async (req: Request) => {
  ...
  if (!q) return Response.json({ error: 'missing "q" query param' }, { status: 400 });
  ...
  while (nextUrl && cards.length < MAX_CARDS) {
    ...
    if (!res.ok) return Response.json({ error: data.details || 'Scryfall request failed' }, { status: 502 });
